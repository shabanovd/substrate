...
impl<T, const S: u32, I> Index<I> for WeakBoundedVec<T, S>
where
	I: SliceIndex<[T]>,
{
...
impl<T, const S: u32, I> IndexMut<I> for WeakBoundedVec<T, S>
where
	I: SliceIndex<[T]>,
{
	#[inline]
...
impl<T, const S: u32> IntoIterator for WeakBoundedVec<T, S> {
	type Item = T;
...
pub mod test {
	...
	crate::generate_storage_alias! { Prefix, Foo => Value<WeakBoundedVec<u32, 7>> }
	crate::generate_storage_alias! { Prefix, FooMap => Map<(Twox128, u32), WeakBoundedVec<u32, 7>> }
	crate::generate_storage_alias! {
		Prefix,
		FooDoubleMap => DoubleMap<(Twox128, u32), (Twox128, u32), WeakBoundedVec<u32, 7>>
	}

	#[test]
	fn bound_returns_correct_value() {
		assert_eq!(WeakBoundedVec::<u32, 7>::bound(), 7);
	}

	#[test]
	fn decode_len_works() {
		TestExternalities::default().execute_with(|| {
			let bounded: WeakBoundedVec<u32, 7> = vec![1, 2, 3].try_into().unwrap();
			Foo::put(bounded);
	...
	#[test]
	fn try_insert_works() {
		let mut bounded: WeakBoundedVec<u32, 4> = vec![1, 2, 3].try_into().unwrap();
	...
	#[test]
	fn try_mutate_works() {
		let bounded: WeakBoundedVec<u32, 7> = vec![1, 2, 3, 4, 5, 6].try_into().unwrap();
	...
	fn too_big_succeed_to_decode() {
		...
		let w = WeakBoundedVec::<u32, 4>::decode(&mut &v.encode()[..]).unwrap();
		assert_eq!(v, *w);
	}
}
